...
def _extract_cookies_from_browser(domains=None):
    if domains is None:
        domains = ['youtube.com', 'facebook.com']   # both domains
    ...
    for domain in domains:
        for name, get_cookies in browsers:
            try:
                print_colored(f"[i] Extracting cookies from {name} for {domain}...", COLORS.BLUE)
                cookies = get_cookies(domain_name=domain)   # single domain
                for cookie in cookies:
                    # skip unnecessary cookies
                    if cookie.name in ('__cf_bm', 'cf_clearance'):
                        continue
                    all_lines.append(_format_cookie(cookie))
            except Exception as e:
                print_colored(f"[!] Failed from {name} for {domain}: {e}", COLORS.RED)
                continue

    if not all_lines:
        return None

    # Remove duplicates (same domain, path, name) – keep first occurrence
    seen = set()
    unique_lines = []
    ...
    cookie_data += "# Generated by browser-cookie3\n\n"
    cookie_data += "\n".join(all_lines)
    return cookie_data
